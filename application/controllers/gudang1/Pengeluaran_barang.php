...
class Pengeluaran_barang extends CI_Controller{
	...
	public function __construct()
	{
		...
		$this->load->model('M_retour_gudang');
	...
	function getDataindex()
	{
		...
		for($i=0;$i<count($order);$i++){
			...
			$data[$i]['LINK_PENGGUNAAN'] = "";
			$data[$i]['SERI'] = $order[$i]->SERI;
			if ($order[$i]->LINK_PENGGUNAAN == '1') {
				$data[$i]['LINK_PENGGUNAAN'] = "KK";
			}
		}
	...
	public function penuhi()
	{
		...
		if($success){
			...
			$_SESSION['pesan'].='<font color="blue">Berhasil disimpan</font>';
				// print_r("<meta http-equiv='refresh' content='0; url=".base_url()."index.php/gudang/pengeluaran_barang'>");
			redirect('gudang/pengeluaran_barang', "refresh");
		}else{
			echo "error";
			exit();
		}
