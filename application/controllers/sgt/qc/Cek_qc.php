...
class Cek_qc extends CI_Controller{
	...
	function index()
	{
		...
		// $tampungDetailPenerimaan = $this->M_detail_penerimaan->getDetailPenerimaanById('3472');
		// $Xbarcode = $tampungDetailPenerimaan[0]->BARCODE;
		// $nomerLabel = substr($Xbarcode, 4, 4);
	...
	public function save()
	{
		...
		foreach ($dataDetail as $key) {
			...
			if ($status === "CLOSE" && $data['grade'] <= 2) { //Close Grade 1 & 2
				...
				// ====== Nomor Label dari database ===============
				// $pqrs = $this->M_nomer->getNomerLabelByTahun($Tahun);
				// $nomerLabel = $pqrs[0]->LABEL_QC;
				// $nomerLabel += 1;
				// ====== Nomor Label dari barcode ================
				$Xid_detail_terima = $this->input->POST('txtIdDetailTerima');
				$tampungDetailPenerimaan = $this->M_detail_penerimaan->getDetailPenerimaanById($Xid_detail_terima);
				$Xbarcode = $tampungDetailPenerimaan[0]->BARCODE;
				$nomerLabel = substr($Xbarcode, 4, 4);
				...
				if ($id_barang == '740' || $id_barang == '4' 
					|| $id_barang == '1093' || $id_barang == '3907' 
					|| $id_barang == '5925' || $id_barang == '7327' 
					|| $id_barang == '7382' || $id_barang == '7565'
					|| $id_barang == '8461' || $id_barang == '9388'
					|| $id_barang == '9471' || $id_barang == '9475' 
					|| $id_barang == '10527' || $id_barang == '11632'
					|| $id_barang == '11918' ) 
