...
<script type="text/javascript">
	...
	$('#modal-detail').on('show.bs.modal', function(e) {
	 	...
	 	var mutasi_id_detail_terima = xyz[5];

	 	txtBarcode.value = xyz[1];
	 	txtMaterial.value = xyz[2];
	 	txtNoPo.value = xyz[3];
	 	txtKodeRoll.value = xyz[6];

	 	$("#tblDetailTest").find("tr:gt(0)").remove(); //gt = kecuali, eq = terpilih
	 	
	 	getTest(mutasi_id_detail_terima,'detail');
	 	getSyarat(xyz[4]);
	})


	$('#modal-recode').on('show.bs.modal', function(e) {
	 	var abc = e.relatedTarget.id;
	 	...
	 	txtBarcodeR.value = xyz[1];
	 	txtMaterialR.value = xyz[2];
	 	txtNoPoR.value = xyz[3];
	 	txtKodeRollR.value = xyz[6];

	 	txtIdD.value = id_detail_terima;
	 	txtStatusQc.value = xyz[7];
	 	txtKode.value = xyz[6];
	 	txtNote.value = "";

	 	$("#tblDetailTestR").find("tr:gt(0)").remove(); //gt = kecuali, eq = terpilih
	 	
	 	getTest(mutasi_id_detail_terima,'recode');
	 	// getSyarat(xyz[4]);
	})
	...
	function getTest(id_detail_terima,Xmodal){
		$.ajax({
        type: 'post',
