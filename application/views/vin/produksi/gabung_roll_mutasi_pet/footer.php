...
function update_kode_roll()
        {
			var pjgRoll=0;
        	var qty_data = tabel_menu.rows.length;
			var kode_roll_gabung= []
			var kode='';
			
        	
			for (var i = 0; i < qty_data - 1; i++) {
				var panjang=tabel_menu.rows[i + 1].cells[5].innerHTML;
			    var	kode_rolls=tabel_menu.rows[i + 1].cells[4].innerHTML;
			    var	digit_roll = kode_rolls.split('-');
	     	    var	qty_digit_roll = digit_roll[0].length;
                var desain = digit_roll[4].substring(2, 4);
				...
				if (status == true) {
		          ...
		          }// kode_roll tidak kosong
        			pjgRoll=pjgRoll+parseInt(panjang);
	                jumlah_gabungan.value=pjgRoll;
	             }
	           else if (status2 == true)	
	            {	
		         panjang=tabel_menu.rows[i + 1].cells[5].innerHTML;
		         pjgRoll=pjgRoll-parseInt(panjang);
	             jumlah_gabungan.value=pjgRoll; 
	            } 	  //akhir else status2
           }// end for
    ...
    if (kode_roll_gabung.length >0)    
      {
	  roll_gabungan.value=kode+''+desain;
	  }
	  else
	  {
	    roll_gabungan.value='';	
		jumlah_gabungan.value='';
	  }
	}
...
function simpan_gabung_roll() {
	...
	var roll_gabungan = document.getElementById('roll_gabungan').value;
	var jumlah_gabungan = document.getElementById('jumlah_gabungan').value;
	var dari = document.getElementById('dari').value;
	var ke = document.getElementById('ke').value;
	var tanggal = document.getElementById('tanggal').value;
	var data = [];
    var cek_roll=0;
	for(var m=0; m<tabel_menu.rows.length - 1; m++){
	var status = document.getElementsByName('pilih_roll')[m].checked;
	var status2 = document.getElementsByName('pilih_roll')[m].unchecked;
	if (status == true) {
	 cek_roll++;	
	}
	else if (status2 == true) {
    ...
    // Validasi isian
    if (cek_roll < 2) {error_isian('Pilih minimal 2 roll untuk penggabungan roll'); return;}
	...
	var status = document.getElementsByName('pilih_roll')[i].checked;
        ...
        var menu = [no_mutasi,kk,id_pengirim,id_penerima,shift,kode_roll,meter,id_prod_mutasi,no_mutasi_lama,tanggal,id_hapus,roll_gabungan,jumlah_gabungan,dari,ke];
		//alert(kk);
		data.push(menu);
	 } // end if
	}// end for
    ...
    $.ajax({
       data: {
        ...
        type: 'POST',
		url: '<?php echo site_url('vin/produksi/mutasi_pet/simpanGabungRoll') ?>',
        success: function(data) {
			data = JSON.parse(data);
			//console.log(data);
