                        ...
                        <table id="tabel_material" class="table table-bordered" width="100%">
                            <thead style="background-color: #3FB4F7; font-weight: bold; color: #FFFFFF;">
                                <tr style="text-align: center;">
                                    ...
                                    <td width="15%">Qty Datang</td>
                                    <td width="15%">Qty PO</td>
                                    <td hidden width="10%">Harga</td>
                                    <td hidden width="7.5%">Mata Uang</td>
                                    <td hidden width="10%">Total</td>
                                    <td hidden></td>
...
function cek_sp(no_sp) {
    ...
    $.ajax({
        ...
        type: 'POST',
        async: false,
        url: '<?php echo base_url() . "index.php/gudang/sj/cek_sp" ?>',
...
$('#btn_material').on('click', function() {
    ...
    $.ajax({
        ...
        type: 'POST',
        url: '<?php echo base_url() . "index.php/gudang/sj/data_po" ?>',
        success: function(data) {
...
function isi_data_po(data_po) {
    ...
    for (var i = 0; i < data_po.length; i++) {
        ...
        urut_sakti = data_po[i].URUT_SAKTI;
        ...
        $('#body_po').append('<tr><td align="center"><input type="checkbox" name="pilih_barang" style="cursor: pointer;"></td><td align="center">' + (i + 1) + '</td><td>' + no_po + '</td><td>' + urut_sakti + '</td><td>' + nama_barang + '</td><td>' + spesifikasi + '</td><td align="right">' + format_number(qty_po) + '</td><td align="center">' + satuan + '</td><td hidden align="right">' + format_number(harga) + '</td><td hidden align="center">' + mata_uang + '</td><td hidden>' + id_po_detail + '</td></tr>');
    }
}
...
$('#btn_pilih').click(function() {
    ...
    for (var i = 0; i < qty_data - 1; i++) {
        ...
        if (status == true) {
            ...
            urut_sakti = tbl_po.rows[i + 1].cells[3].innerHTML;
            nama_barang = tbl_po.rows[i + 1].cells[4].innerHTML;
            spesifikasi = tbl_po.rows[i + 1].cells[5].innerHTML;
            qty_po = tbl_po.rows[i + 1].cells[6].innerHTML;
            satuan = tbl_po.rows[i + 1].cells[7].innerHTML;
            harga = tbl_po.rows[i + 1].cells[8].innerHTML;
            mata_uang = tbl_po.rows[i + 1].cells[9].innerHTML;
            id_po_detail = tbl_po.rows[i + 1].cells[10].innerHTML;
...
function isi_material(no_po, nama_barang, spesifikasi, qty_po, satuan, harga, mata_uang, id_po_detail, qty_datang) {
    ...
    $('#tabel_material').append(
        ...
        '<td hidden><input type="text" class="form-control" name="harga" value="' + harga + '" style="width: 100%; text-align: right;" readonly></td>' +
        '<td hidden><input type="text" class="form-control" value="' + mata_uang + '" style="width: 100%; text-align: center;" readonly></td>' +
        '<td hidden><input type="text" class="form-control" name="total" style="width: 100%; text-align: right;" readonly></td>' +
...
function total_datang(id_po_detail) {
    ...
    $.ajax({
        ...
        data: {data: data},
        type: 'POST',
        url: '<?php echo base_url() . "index.php/gudang/sj/total_datang" ?>',
        success: function(data) {
            total = data;
        }
    });
...
function simpan() {
    ...
    $.ajax({
        ...
        data: {data: data},
        type: 'POST',
        url: '<?php echo base_url() . "index.php/gudang/sj/simpan" ?>',
        success: function(data) {
            console.log(data);
            setTimeout(function() {
                $('#btnOk').click();
...
function filter() {
    ...
    $.ajax({
        data: {data: data},
        type: 'POST',
        url: '<?php echo base_url() . "index.php/gudang/sj/filter" ?>',
        success: function(data) {
...
function cek_batal(id_detail) {
    $.ajax({
        async: false,
        type: 'POST',
        url: '<?php echo base_url(); ?>index.php/gudang/sj/cek_batal',
        data: {data: id_detail},
        success: function(data) {
...
function batal(btn) {
    ...
    $('#btnYa').on('click', function() {
        ...
        $.ajax({
            async: false,
            type: 'POST',
            url: '<?php echo base_url(); ?>index.php/gudang/sj/batal',
            data: {data: id_hapus},
            success: function(data) {
                setTimeout(function() {
                    $('#btnOk').click();
...
function edit(btn) {
    ...
    $.ajax({
        async: false,
        type: 'POST',
        url: '<?php echo base_url(); ?>index.php/gudang/sj/edit',
        data: {data: id_detail},
        success: function(data) {
