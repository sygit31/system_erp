...
function simpan() {
	...
	for (var i=0; i<qty_data; i++) {
		...
		start = cek_jam(tanggal, t_mulai);
		end = cek_jam(tanggal, t_selesai);
...
function cetak_emboss(data) {
	...
	if (proses == 'Emboss') {
		for (var i=0; i<dt_pch.length; i++) {
			bahan = dt_pch[i].NAMA;
			qty = desimal(dt_pch[i].QTY.replace(',','.')) + ' ' + dt_pch[i].SATUAN;
...
function cetak_belah(data) {
	...
	lebar_bahan = '36,5', t_hasil = 0, t_pita = 0, t_rusak = 0;
	for (var i=0; i<dt_proses.length; i++) {
		...
		if (dt_proses[i].SERI != 'MMEA') {
			seri = dt_proses[i].SERI.split(' ')[1];
			lebar = seri == 'I' ? '0.7' : '0.5';
		}else{
			seri = dt_proses[i].SERI;
			lebar = '0.6';
		}
...
function edit(btn) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			$('#e_bahan').val(format_number(dt_roll.BAHAN.replace(',', '.'))).change();
...
function simpan_edit() {
	...
	var data = [id_edit, tgl, shift, mulai, selesai, hasil, reject, sisa, bahan, mesin, operator, pengawas];
