							...
							<table style="width: 1000px;">
								...
								<tbody>
									<tr>
										...
										<td></td>
										<td>
											<select class="select_min" id="fMesin" onchange="filter()" style="width: 100%;">
											</select>
										</td>
									</tr>
								</tbody>
			...
			success: function(data) {			
				...
				setTimeout(function() {
					...
					for (var i=0; i<dt_tgl_prod.length; i++) {
						...
						hasil = dt_tgl_prod[i].HASIL == null ? '' : (format_number(dt_tgl_prod[i].HASIL));
						...
						$('#data-table tbody tr:eq('+i+')').append('<td align="center">'+hasil+'</td><td align="center"></td>');
					}
				...
				// Isi Total Jam (Horizontal)
					for (var i=0; i<qty_tgl-2; i++) {
						dt_jam = 0;
						hasil = $('#data-table tbody tr:eq('+i+') td:eq(14)').html();
						...
						speed = Number(angka(hasil)) / jam_efektif / 60;

						$('#data-table tbody tr:eq('+i+') td:eq(3)').html(jam_efektif.toFixed(2));
						$('#data-table tbody tr:eq('+i+') td:eq(15)').html(speed.toFixed(2));
					}
				...
				// Isi Total Jam (Vertikal)
					for (var i=2; i<qty_kode+2; i++) {
						...
						$('#data-table tfoot tr:eq(0) td:eq('+i+')').html(format_number(dt_jam.toFixed(2)));
					}
					t_hasil = angka($('#data-table tfoot tr:eq(0) td:eq('+qty_kode+')').html());
