...
<style>
    ...
    @media print {
        ...
        html, body {
            width: 330mm;height: 210mm;
        }

        #pr_body td {
            height: 20px; vertical-align: middle; padding-left: 5px;
        }
    }
...
<div id="printable" style="display: none; overflow: hidden; font-size: 13px; margin-left: 27mm;">
    ...
    <table id="pr_detail" class="table-bordered mt-2" width="100%" style="text-align: center;">
        <thead>
            <tr>
                <td>Rincian Waste</td>
            </tr>
            <tr>
                ...
                <td style="width: 60px; font-size: 10px;">Label</td>
                <?php foreach ($waste->result_array() as $dt) { ?>
                    <td><?php echo $dt['KD_REJECT']; ?></td>
                <?php } ?>
...
// Isi Summary Waste
    function isi_summary(dt_tgl, data) {
        ...
        $('.tbl_summary:eq('+qty_tbl+') tbody').append('<tr class="font-weight-bold"><td>Total</td><td>'+format_number(t_baik)+'</td><td>'+format_number(t_waste_h)+'</td><td></td><td>'+format_number(t_waste_k)+'</td><td></td><td>'+format_number(t_waste)+'</td><td>'+format_number(p_waste)+'</td></tr>');
    }
    ...
    function cetak(btn) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#pr_detail').css('width', kd_reject.length * 55 + 'px');
