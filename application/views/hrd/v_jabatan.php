	...
	<section class="content">
		<div class="card card-info">
			<div class="card-header">
				<h3 class="card-title">
					<b><font color="White"><div id="headerinput">Input Data Jabatan</div></font></b>
				</h3>
            ...
            <div class="card-body">
                <table width="100%">
                    <tr>
                        <td width="20%"><label><font size = "3">Kode</font></label></td>
                        <td width="40%">
                            <input type="text" class="form-control" id="kode" style="width: 60%; text-transform: uppercase;" autocomplete="off" tabindex="1">
                        </td>                        
                        <td width="20%"><label><font size = "3">Level Jabatan</font></label></td>
                        <td width="20%">                            
                            <select class="select" id="level" style="width: 100%;">
                                <option value="">Level Jabatan..</option>
                                <option>1 - Manajemen</option>
                                <option>2 - Supervisor</option>
                                <option>3 - Staff</option>
                                <option>4 - Pelaksana</option>
                    ...
                    <tr height="10"></tr>
                    <tr>
                        <td><label><font size = "3">Nama Jabatan</font></label></td>
                        <td>
                            <input type="text" class="form-control" id="jabatan" autocomplete="off" style="width: 80%; text-transform: uppercase;" tabindex="2">
                        </td>
                    </tr>
                </table>
            </div>
        ...
        <div class="card card-info">
            <div class="card-header">
                <h3 class="card-title">
                    <b><font color="White">Data jabatan</font></b>
                </h3>
                <div class="card-tools">
                    <button type="button" class="btn btn-tool info_1" data-widget="collapse" data-toggle="tooltip" title="Collapse"><i class="fa fa-minus info_1"></i></button>
                    <button type="button" class="btn btn-tool" data-widget="remove" data-toggle="tooltip" title="Remove"><i class="fa fa-times"></i></button>
                            ...
                            <table style="width: 30%; margin-bottom: 10px;">
                                <thead>
                                    <tr align="center" style="line-height: 30px;">
                                        <td width="100%" class="filter">Cari Jabatan</td>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td>
                                            <input type="text" id="cari" onkeyup="filter()" placeholder="Cari nama jabatan.." style="width: 100%;" autocomplete="off">
                                        </td>
                                    </tr>
                                </tbody>
                            </table>

                            <?php $this->load->view('hrd/v_jabatan_table'); ?>
...
function filter() {
    ...
    $.ajax({
        type: 'POST',
        url:'<?php echo base_url(); ?>index.php/hrd/jabatan/filter',
        data: {data: cari},
...
function simpan() {
    ...
    var jabatan = $('#jabatan').val().toUpperCase();
    var level = $('#level').val();
    var data = [kode, jabatan, level, id_edit];

    if (kode == '' || jabatan == '' || level == '') {        
        $('#btnIsian').click();
        return;
    }
    ...
    $.ajax({
        type: 'POST',
        url:'<?php echo base_url(); ?>index.php/hrd/jabatan/simpan',
        data: {data: data},
        success: function(data) {
            setTimeout(function() {
                $('#btnOk').click();
...
function edit(btn) {
    ...
    var jabatan = table.rows[row].cells[3].innerHTML;
    var level = table.rows[row].cells[4].innerHTML;
