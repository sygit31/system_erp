...
<section class="content">
    ...
    <div class="card card-info">
        ...
        <div class="card-body">
                        ...
                        <table style="width: 1100px; margin-bottom: 10px;">
                            <thead>
                                <tr align="center" style="line-height: 30px;">
                                    ...
                                    <td></td>
                                    <td width="17%" class="filter">Pemilik Dokumen</td>
                                    <td></td>
            ...
            <div class="card-footer">
                <button type="button" class="btn btn-block m-3 btn-danger" onclick="hapus_semua()" style="width: 150px;"><i class="fa fa-trash m-2"></i><b>Hapus</b></button>
            </div>
        </div>
    </div>
...
<script>
    ...
    function ambil_file(btn) {
        ...
        var id = data_table.rows[row].cells[9].innerHTML;
        return [file, id];
    }
    ...
    function preview(btn) {
        ...
        var data_table = document.getElementById('data-table');
        var row = $(btn).closest("tr").index() + 1;
        var id_offline = data_table.rows[row].cells[9].innerHTML;
    ...
    // Pilih Semua Data
    function pilih_semua(btn) {
        var qty_data = $('#data-table tbody tr').length;
        var pilih = btn.checked;

        for (var i=0; i<qty_data; i++) {
            document.getElementsByName('pilih')[i].checked = (pilih == true ? true : false);
        }
    }
    ...
    function pilih_satu() {
        ...
        for (var i=0; i<qty_data; i++) {
            pilih = document.getElementsByName('pilih')[i].checked;
            qty_pilih = pilih == true ? qty_pilih+1 : qty_pilih;
        }

        $('#pilih').prop('checked', qty_pilih == qty_data);
    }
    ...
    function hapus_semua() {
        ...
        for (var i=0; i<qty_data; i++) {
            pilih = document.getElementsByName('pilih')[i].checked;
            id_data = $('#data-table tbody tr:eq('+i+') td:eq(9)').html();
            ext = $('#data-table tbody tr:eq('+i+') td:eq(14)').html();
            filename = 'images/bank_data/' + id_data + '.' + ext;

            if (pilih == true) {dt_hapus.push(id_data+'@@'+filename);}
        }

        if (dt_hapus.length == 0) {salah_isian('Pilihan hapus');}
