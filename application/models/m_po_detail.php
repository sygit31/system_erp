<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class m_po_detail extends CI_Model 
	{
		...
		{
			$sql = "SELECT PD.ID,P.NOMER,S.NAMA NAMA_SUPPLIER,B.NAMA NAMA_BARANG,PD.QTY,PD.SATUAN,P.TGL FROM ERP_PO_DETAIL PD JOIN ERP_PO P ON PD.ID_PO = P.ID JOIN ERP_BAGIAN MB ON P.ID_BAGIAN = MB.ID JOIN ERP_MATERIAL_SUPPLY MS ON PD.ID_MATERIAL_SUPPLY = MS.ID JOIN ERP_BARANG B ON MS.ID_BARANG = B.ID JOIN ERP_SUPPLIER S ON MS.ID_SUPPLIER = S.ID WHERE PD.STATUS = 'OTW'";
