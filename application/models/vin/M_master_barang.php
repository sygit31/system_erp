<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class M_master_barang extends CI_Model {
		...
		public function getTahunByIdDetailTerima($data) {
			...
			// $sql = "SELECT b.TAHUN,b.KODE,b.ID FROM ERP_BARANG b JOIN ERP_MATERIAL_SUPPLY ms ON ms.ID_BARANG = b.ID JOIN ERP_PO_DETAIL pd ON pd.ID_MATERIAL_SUPPLY = ms.ID JOIN ERP_PENERIMAAN p ON p.ID_PO_DETAIL = pd.ID JOIN ERP_PENERIMAAN_DETAIL dp ON p.ID_TERIMA = dp.ID_TERIMA where dp.ID_DETAIL_TERIMA = ". $data;
			$sql = "SELECT b.TAHUN,b.ID KODE,b.ID FROM ERP_BARANG b JOIN ERP_MATERIAL_SUPPLY ms ON ms.ID_BARANG = b.ID JOIN ERP_PO_DETAIL pd ON pd.ID_MATERIAL_SUPPLY = ms.ID JOIN ERP_PENERIMAAN p ON p.ID_PO_DETAIL = pd.ID JOIN ERP_PENERIMAAN_DETAIL dp ON p.ID_TERIMA = dp.ID_TERIMA where dp.ID_DETAIL_TERIMA = ". $data;
