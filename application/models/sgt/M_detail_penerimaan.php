...
class M_detail_penerimaan extends CI_Model 
{
	...
	public function getRollBonProduksi() 
	{
		$sql = "select a.*,br.tahun,pe.id_gudang_order from erp_penerimaan_detail a
        LEFT JOIN erp_prod_pet_detail_terima b ON a.ID_DETAIL_TERIMA = b.ID_DETAIL_TERIMA
        JOIN ERP_PENERIMAAN p ON P.ID_TERIMA = a.ID_TERIMA
        join erp_po_detail pd on p.id_po_detail = pd.id
        join erp_material_supply ms on pd.id_material_supply = ms.id
        join erp_barang br on ms.id_barang = br.id
        join erp_ipb_detail idx on a.ID_DETAIL_TERIMA = idx.ID_DETAIL_TERIMA
        join erp_ipb ipb on ipb.id = idx.id_ipb
        join erp_pengeluaran pe on pe.id_ipb = ipb.id
        WHERE b.ID_PROD_PET_DETAIL IS NULL
        AND a.KODE_ROLL IS NOT NULL
        AND a.STATUS_QC = 'OUT'
        AND KODE_ROLL NOT LIKE '%-2021'
        AND KODE_ROLL NOT LIKE '%-2020'
        AND KODE_ROLL NOT LIKE '%-2019'
        ORDER BY a.KODE_ROLL";
