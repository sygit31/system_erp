...
class M_packing extends CI_Model {
	...
	function auto_no($id_edit, $desain, $tgl, $tipe) {
		if ($id_edit != '') {
			$query = $this->db->query("Select nmr, desain, tipe from erp_qc_packing where id='$id_edit'");
		...
		$query = $this->db->query("Select max(nmr) nmr, to_char(max(tgl), 'DD-MM-YYYY') tgl from erp_qc_packing where desain='$desain' and tipe='$tipe'");
		$data = $query->row_array();
	...
	function simpan($urut, $nmr, $desain, $tgl, $jam, $mesin, $produk, $cutter, $sortir, $qc, $packing, $id_pemeriksa, $id_approval, $id_pengawas, $hasil_baik, $total, $rim, $sampling, $plus, $mins, $ku, $holo, $kts, $remark, $tipe) {
		$this->db->query("Insert into erp_qc_packing(id, desain, nmr, tgl, jam, mesin_hitung, cutter, produk, kode_sortir, kode_qc, kode_packing, hasil_baik, rim_baik, rim_sampling, plus, mins, ku, holo, kts, total, id_pemeriksa, id_pengawas, id_approval, remark, tipe) values('$urut', '$desain', '$nmr', '$tgl', to_date('$jam','DD-MM-YYYY HH24:MI:SS'), '$mesin', '$cutter', '$produk', '$sortir', '$qc', '$packing', '$hasil_baik', '$rim', '$sampling', '$plus', '$mins', '$ku', '$holo', '$kts', '$total', '$id_pemeriksa', '$id_pengawas', '$id_approval', '$remark', '$tipe')");
	}

	function update($id_edit, $nmr, $desain, $tgl, $jam, $mesin, $produk, $cutter, $sortir, $qc, $packing, $id_pemeriksa, $id_approval, $id_pengawas, $hasil_baik, $total, $rim, $sampling, $plus, $mins, $ku, $holo, $kts, $remark, $tipe) {
		$this->db->query("Update erp_qc_packing set desain='$desain', nmr='$nmr', tgl='$tgl', jam=to_date('$jam','DD-MM-YYYY HH24:MI:SS'), mesin_hitung='$mesin', cutter='$cutter', produk='$produk', kode_sortir='$sortir', kode_qc='$qc', kode_packing='$packing', hasil_baik='$hasil_baik', rim_baik='$rim', rim_sampling='$sampling', plus='$plus', mins='$mins', ku='$ku', holo='$holo', kts='$kts', total='$total', id_pemeriksa='$id_pemeriksa', id_pengawas='$id_pengawas', id_approval='$id_approval', remark='$remark', tipe='$tipe' where id='$id_edit'");
	}
	...
	function cetak($id_cetak) {
		$query = $this->db->query("Select qo.nmr, to_char(qo.tgl, 'DD-MM-YYYY') tgl, to_char(qo.jam,'hh24:mi') jam, qo.cutter, qo.produk, qo.kode_sortir, qo.kode_qc, qo.kode_packing, qo.hasil_baik, qo.rim_baik, qo.rim_sampling, qo.plus, qo.mins, qo.total, qo.ku, qo.holo, qo.kts, qo.remark, nvl(ha.nick_name, ha.nama) approval, nvl(ha2.nick_name, ha2.nama) pemeriksa, nvl(ha3.nick_name, ha3.nama) pengawas,
			...
			where qo.nmr=(Select nmr from erp_qc_packing where id='$id_cetak') and qo.desain=(Select desain from erp_qc_packing where id='$id_cetak') and qo.tipe=(Select tipe from erp_qc_packing where id='$id_cetak')
		...
		$query_su = $this->db->query("Select produk, sum(bahan) bahan, sum(baik) baik, sum(temuan) temuan
			from erp_qc_packing_su where tgl=(select tgl from erp_qc_packing where id='$id_cetak') and desain=(select desain from erp_qc_packing where id='$id_cetak') and tipe=(select tipe from erp_qc_packing where id='$id_cetak')
