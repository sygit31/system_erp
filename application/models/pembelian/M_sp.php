...
class M_sp extends CI_Model
{
    ...
    function filter($tgl1, $tgl2, $kd_unit, $kategori, $jenis, $nmr) {
        return $this->db->query("Select pm.id id_detail_sp, pe.nama supplier, to_char(pl.tgl,'dd-mm-yyyy') tgl,ce.no_sip, pl.nmr, pc.jenis, case when pc.kode_sakti is null then pc.nama when kode_sakti is not null then b.nama_barang_sakti end nama_barang, pc.spesifikasi, pc.satuan, pm.qty qty_sp, pa.nomer, pb.harga, pb.qty qty_po, ha.nama kary,
    ...
    function data_po($id_supplier, $kd_unit) {
        $query = $this->db->query("select * from(Select pa.nomer no_po, pe.nama supplier, case when pc.kode_sakti is null then pc.nama when kode_sakti is not null then b.nama_barang_sakti end nama_barang, pc.spesifikasi, pb.qty qty_po, pb.satuan, pb.harga, pd.mata_uang, pb.id id_po_detail,
