...
class M_ipb extends CI_Model 
{
	...
	public function getIpbOrderByIdKK($id_kk_detail) 
	{
		...
		JOIN ERP_IPB_DETAIL ID ON I.ID=ID.ID_IPB
		JOIN ERP_KK_DETAIL KD ON I.ID_KK_DETAIL = KD.ID
		WHERE I.ID_KK_DETAIl = ".$id_kk_detail." AND ID.STATUS = 'ORDER'
		ORDER BY NOMER";
	...
	public function getCetakById($id_ipb) 
	{
		...
		WHERE I.ID = ". $id_ipb."
		ORDER BY PD.KODE_ROLL";

		$query = $this->db->query($sql);
		return $query->result();
	}

	public function getAllIPB($edit=false,$data=Array()) 
	{
		...
		$syntaxWhere = 'ADD_MONTHS(SYSDATE, -3) AND SYSDATE';

		if ($edit) {
			$syntaxWhere = "'".$data['tglAwal']."' AND '".$data['tglAkhir']."'";
		}
		...
		JOIN ERP_KK_DETAIL KD ON I.ID_KK_DETAIL = KD.ID
		JOIN ERP_KK K ON KD.ID_KK = K.ID
		WHERE TANGGAL BETWEEN ".$syntaxWhere."
		ORDER BY I.TANGGAL DESC,PD.KODE_ROLL DESC";
